...
(
   ...
   FKFZ                 CHAR(6),
   LQZJSJ               CHAR(8),
   RET_CHKFILE_NAME VARCHAR(256),
   RET_CHKMD5_CODE VARCHAR(60),
   TH_STAT CHAR(2),
...
COMMENT ON COLUMN AGENT_BATCH_INFO.WJLX
IS
    '文件类型';
COMMENT ON COLUMN AGENT_BATCH_INFO.ZDRZBZ
IS
    '自动入账标志 0-否 1-是';
COMMENT ON COLUMN AGENT_BATCH_INFO.ZXSJ
IS
    '执行时间';
COMMENT ON COLUMN AGENT_BATCH_INFO.FKFZ
IS
    '发卡/发折标志';
COMMENT ON COLUMN AGENT_BATCH_INFO.LQZJSJ
IS
    '领取证件时间';
COMMENT ON COLUMN AGENT_BATCH_INFO.RET_CHKFILE_NAME
IS
    '校验回盘文件';
COMMENT ON COLUMN AGENT_BATCH_INFO.RET_CHKMD5_CODE
IS
    '校验回盘文件MD5';
COMMENT ON COLUMN AGENT_BATCH_INFO.TH_STAT
IS
    '退回状态';
