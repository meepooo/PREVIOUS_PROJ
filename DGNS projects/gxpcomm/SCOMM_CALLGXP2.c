...
IRESULT SCOMM_CALLGXP3(HXMLTREE lXmlhandle)
{
    ...
    LOG(LM_DEBUG,Fmtmsg("COMMBUF LEN -->[%d]", nLen),"")
		...
		if (( len= xml_RelGetBinElement( lXmlhandle, sHeadName, psBuffer, nLen+iheadlen)) == FAIL )
		{
				fpub_SetMsg(lXmlhandle,MID_SYS_COMPEXEC, Fmtmsg(MSG_SYS_COMPEXEC,"执行xml_RelGetBinElement错误！!!!!!!!"));
				fpub_SetCompStatus(lXmlhandle,1);
				LOG(LM_STD,Fmtmsg("执行xml_RelGetBinElement错误！!!!!!!!!!"),"")
				iRet = 1;
				goto err;
		}
		...
		/*发送请求报文*/
		LOG(LM_DEBUG,Fmtmsg("申请动态内存len=[%d]", iheadlen),"")
#ifdef  DEBUG
		LOG(LM_DEBUG,Fmtmsg("需要发送的报文数据包len=[%d]", len),"")
#endif
    ...
    /*接收对方应答包头*/
		...
		if (iRet < 0)
		{
				...
				fpub_SetMsg(lXmlhandle,MID_SYS_COMPEXEC, Fmtmsg(MSG_SYS_COMPEXEC,"接收GXP后台报文失败"));
				fpub_SetCompStatus(lXmlhandle,2);
				LOG(LM_STD,Fmtmsg("接收GXP后台报文长度失败"),"")
				iRet = 2;
				goto err;
		}
		...
		{
				LOG(LM_STD,Fmtmsg("请动态内存失败"),"")
				iRet = 1;
				goto err;
		}
		LOG(LM_DEBUG,Fmtmsg("申请动态内存len=[%d]", len+1),"")
		...
		if (iRet < 0)
		{
				...
				fpub_SetMsg(lXmlhandle,MID_SYS_COMPEXEC, Fmtmsg(MSG_SYS_COMPEXEC,"接收GXP后台报文失败"));
				fpub_SetCompStatus(lXmlhandle,2);
				LOG(LM_STD,Fmtmsg("接收GXP2后台报文内容失败"),"")
				iRet = 2;	
				goto err;
		}
    LOG(LM_DEBUG,Fmtmsg("接收到的长度len=[%d]",iRet),"")
#ifdef  DEBUG
    LOG(LM_DEBUG,Fmtmsg("接收到的报文数据包=[%s]", psPkgIn),"")
#endif
		...
		*(psBuffer+iheadlen+1)='\0';
		...
		if(xml_SetBinElement(lXmlhandle,sHeadName,psBuffer,iheadlen)<0)
		{
				...
				fpub_SetMsg(lXmlhandle,MID_SYS_COMPEXEC,Fmtmsg(MSG_SYS_COMPEXEC,"执行xml_RelSetBinElement错误！"));
				fpub_SetCompStatus(lXmlhandle,2);
				LOG(LM_STD,Fmtmsg("执行xml_RelSetBinElement错误！"),"")
				iRet = 2;
				goto err;
		}
		if(len > 0)
