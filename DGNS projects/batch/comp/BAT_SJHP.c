...
int ShowSocompVersion(char *sinfo,int isize)
{
    char sversion[]="<?xml version=\"1.0\" encoding=\"GB2312\"?>\
        <root>\
        <soname>BAT_SJHP.so</soname>\
        <sosrcname>BAT_SJHP.c</sosrcname>\
        <sonote>BATCH数据回盘组件集合</sonote>\
        <version>\
        <rec ver=\"2.1.0.1\" modify=\"2011-8-1 15:08\" programer=\"YangDong\" filesize=\"1291580\">HS-AgentV2.1整理</rec>\
        <rec ver=\"4.1.0.1\" modify=\"2014-10-18 15:08\" programer=\"YangDong\" filesize=\"1291580\">DCI改造</rec>\
        </version>\
        </root>";
    printf("\n================动态组件基本信息================\n");
...
参数使用示例:
************************************************************************/
IRESULT SBATCH_SJHP(HXMLTREE lXmlhandle)
{
    char sMapID[40+1];
    char sColID[32+1];
    int  iHpwjwz;
    ...
    char sCondStr[256];   /* 回盘条件字符串 */
    /* mod by yangdong 2030305 文件列数多时长度可能不够 */
    /* 回盘文件所需列名 */
    /*
    char sColStr[128];
    char sSqlStr[512];
    */
    char sColStr[1024];    /* 回盘文件所需列名 */
    char sSqlStr[1300];
    long lRCount=0;
    int  iTotalNum=0;
    int  iLsxh = 0;
    ...
    COMP_GETPARSEPARAS(2,sBuf,"回盘文件名")
    ...
    LOG(LM_DEBUG,Fmtmsg("BATCH: 回盘标志[%s] 文件标识[%s] 文件名[%s] 批次号[%s]",\
        sGenFlag, sMapID, sFileBasicName, sPtpch  ),"DEBUG");
    ...
    if( strlen( sFileBasicName ) < 1 )
    {
        if( sGenFlag[0] == '3' )  /* 失败文件，默认加_f */
            snprintf( sFileBasicName, sizeof( sFileBasicName ),\
                "%s_%s_%s_f", sYwbh, sPkrq, sPtpch );
        else 
            snprintf( sFileBasicName, sizeof( sFileBasicName ),\
               "%s_%s_%s", sYwbh, sPkrq, sPtpch );
    }

    if( strlen( sGenFlag ) < 1 )
        strcpy( sGenFlag,"1" );  /* 默认回盘标志 1 本批全部明细  */

    /* 组装回盘文件需要处理字符串 */
    /* 根据映射配置，取需要处理的字段 */
    memset( sSql1, 0x00, sizeof( sSql1 ) );
    snprintf( sSql1, sizeof( sSql1 ), "SELECT * FROM  batch_filemap\
        WHERE mapid ='%s' AND hpwjwz > 0 order by hpwjwz ",sMapID  );
    ...
    while( ( iRet = DBFetch(cur, SD_BATCH_FILEMAP, NUMELE(SD_BATCH_FILEMAP), &sdb_batch_filemap, sErrmsg ) ) > 0 )
    {
        iTotalNum++;
#if 0
delete by yangdong 20130609同数据提交，存在值设置或者常量时，不处理
        /* added by yangdong 20121108 在select字段中添加空值，保证导出文件字段数与文件配置匹配 */
        if( iTotalNum < iHpwjwz )
        {
            while( iHpwjwz > iTotalNum && iTotalNum < BATCH_MAX_FILE_COL_NUM )
            {
                strcat( sColStr, " ''" );
                strcat( sColStr, "," );
                iTotalNum++;
    ...
    /* 回盘文件名解析( 配置信息可以带xml节点名 ) */
    if( strlen( sFileBasicName ) > 2 )
    {
        LOG(LM_DEBUG,Fmtmsg("BATCH: 回盘文件名称解析,[%s]=>",sFileBasicName ),"DEBUG")
        /* 先解析XML变量 */
    ...
    LOG(LM_DEBUG,Fmtmsg("BATCH: 开始处理回盘文件[%s][%s]", sFilePath, sFileBasicName ),"DEBUG");
    /* 文件路径处理 */
    ...
    LOG(LM_DEBUG,Fmtmsg("BATCH: 开始处理回盘文件[%s]type[%s]", sFileFullName,sFileType ),"DEBUG");

    /* 统计核心实际交易金额 */
    iTotalNum = 0;
    if( sGenFlag[0] == '3' )  /* 失败明细信息 则统计交易金额 */
    ...
    LOG(LM_DEBUG,Fmtmsg("BATCH: 回盘条件[%s]", sSqlStr ),"DEBUG");
    memset(sTmpFileName,0,sizeof(sTmpFileName));
    snprintf(sTmpFileName, sizeof( sTmpFileName ),\
             "%s/tmp/batch_sjhp_tmp%s.%s", getenv("HOME"), sPkrq, sPtpch );
    ...
    if( iAbort )
    {
        LOG(LM_STD,Fmtmsg("BATCH: 生成回盘文件处理错误: [%d]",iAbort),"ERROR");
    ...
    /* 设置文件信息 */
    if( sGenFlag[0] == '3' )  /* 失败明细信息 */
    {
        /* 文件名 */
        COMP_SOFTSETXML( XMLNM_BATCH_SJHP_SBWJ, sFileBasicName );
        /* 文件内笔数 */
        COMP_SOFTSETXML( XMLNM_BATCH_SJHP_SBZBS, itoa( iTotalNum ) );
        /* 文件内交易金额 */
        sprintf( sBuf, "%.2f", dTotalAmt );
        COMP_SOFTSETXML(  XMLNM_BATCH_SJHP_SBZJE, sBuf );
    }
    else /* 成功明细信息 */
    {
        /* 文件名 */
        COMP_SOFTSETXML( XMLNM_BATCH_SJHP_CGWJ, sFileBasicName );
        /* 文件内笔数 */
        COMP_SOFTSETXML( XMLNM_BATCH_SJHP_CGZBS, itoa( iTotalNum ) );
        /* 文件内交易金额 */
        sprintf( sBuf, "%.2f", dTotalAmt );
        COMP_SOFTSETXML(  XMLNM_BATCH_SJHP_CGZJE, sBuf );
    }

    LOG(LM_DEBUG,Fmtmsg("BATCH:数据回盘,文件名称[%s],总笔数[%d],总金额[%.2f]",\
    ...
    LOG(LM_STD,Fmtmsg("BATCH: 数据回盘转换编码格式,文件类型[%s] 文件格式[%s] 编码格式[%s]", 
