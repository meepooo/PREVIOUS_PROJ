...
#endif
#define ICPS_GET_MONEY_FROM_TOTALFILE_JB(sFile, dTo, sName) {             \
    memset(sFileLineUTF8, 0, sizeof(sFileLineUTF8));                      \
    memset(sFileLineGBK, 0, sizeof(sFileLineGBK));                        \
    memset(sBuf, 0, sizeof(sBuf));                                        \
    fgets(sFileLineUTF8, sizeof(sFileLineUTF8), sFile);                   \
    chomp_CRLF(sFileLineUTF8);                                            \
    code_convert("UTF-8", "GBK", pUTF8, sizeof(sFileLineUTF8), pGBK, sizeof(sFileLineGBK)); \
    GetListbyDiv(sFileLineGBK, 2, sBuf, ',', '\\');                       \
    trim(sBuf);                                                           \
    dTo = atoi(sBuf) / 100.0;                                             \
}
...
#define ICPS_WB_GET_STRING_BY_DIV(sFrom, iListno, sTo, cDiv, sName) {     \
    memset(sBuf, 0, sizeof(sBuf));                                        \
    GetListbyDiv(sFrom, iListno, sBuf, cDiv, 22);                       \
    trim(sBuf);                                                           \
    if (0 == strcmp(sBuf, "\\N"))                                         \
        sTo[0] = 0;                                                       \
    else                                                                  \
