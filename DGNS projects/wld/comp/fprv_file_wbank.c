...
int ffunc_parse_wbank_file_to_db_tm_account(HXMLTREE lXmlhandle, PARA_CHKFILEINS st_para_chkfileins)
{
   ...
   LOG(LM_STD, Fmtmsg("打开隐藏文件结束[%s]", list_file_name), "ERROR")
    while (!feof(fp_list))
    {
        ...
        while (!feof(fp_check))
        {
        	...
        	LOG(LM_STD, Fmtmsg("UTF-8转GBK结果[%s]",sFileLineGBK), "DEBUG")
            rcdCnt++;
            transcCnt++;
...
{
    int iRet;
    LOG(LM_STD, Fmtmsg("function-ffunc_init_wbank_file_info"), "INFO")
