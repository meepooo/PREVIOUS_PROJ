...
int ffunc_mxzz_check(HXMLTREE lXmlhandle, char* subject, char* busi_date, char* flag, double* je)
{
   	...
   	LOG(LM_STD, Fmtmsg("iRje=[%s]", iRje), "ERROR")
    	...
    	if (0==strcmp(sFlag,"D"))
    	{
			...
			else
		  {
    	 	...
    	 	LOG(LM_DEBUG, Fmtmsg("查询会记总账表指定科目当日发生额[%s],方向[%s]", sSqlStr, sFlag), "DEBUG");
         iRet = DBSelectToVarChar(sErrmsg, sBuf, sSqlStr);
         if (iRet < 0)
    	 {
        	LOG(LM_STD, Fmtmsg("数据库操作失败,sqlerr[%s]", DCILastError()), "ERROR")
	  ...
	  {
	  	LOG(LM_STD, Fmtmsg("科目[%s]、方向[%s] 总金额等于汇总金额", sSubject, sFlag), "DEBUG")
	  	*je=iHje;
		return 0;
	  }
	  else if(0<strncmp(sZje, sHje, sizeof(sHje)))
	  {
	  	LOG(LM_STD, Fmtmsg("科目[%s]、方向[%s] 总金额小于汇总金额", sSubject, sFlag), "DEBUG")
	  	*je=iHje;
	  	return 1;
	  }	
	  else  
	  {
	  	LOG(LM_STD, Fmtmsg("科目[%s]、方向[%s] 总金额大于汇总金额", sSubject, sFlag), "DEBUG")
	  	*je=iHje;
	  	return 2;
	  }
}  
