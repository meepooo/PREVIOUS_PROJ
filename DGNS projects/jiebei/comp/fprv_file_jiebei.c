...
int ffunc_parse_jiebei_file_to_db_accounting(HXMLTREE lXmlhandle, FILE* fp, PARA_CHKFILEINS st_para_chkfileins)
{
    int iRet = -1;
    char sBuf[255];
    char sFileLineUTF8[1024], sFileLineGBK[1024], sFileBufUTF8[3*1024], sFileBufGBK[3*1024];
    char *pUTF8 = sFileBufUTF8, *pGBK = sFileBufGBK, *pBuf=NULL;
    char sSqlStr[1024];
    char sErrmsg[256];
...
LOG(LM_STD, Fmtmsg("==========BEGIN==========="), "INFO");
	memset(sFileBufUTF8, 0, sizeof(sFileBufUTF8)) ;
	memset(sFileBufGBK, 0, sizeof(sFileBufGBK)) ;
	fread(sFileBufUTF8, sizeof(sFileBufUTF8), 1, fp) ;
	chomp_CRLF(sFileLineUTF8);
	code_convert("UTF-8", "GBK", pUTF8, sizeof(sFileBufUTF8), pGBK, sizeof(sFileBufGBK));
LOG(LM_STD, Fmtmsg("sFileBufGBK[%s]", pGBK), "INFO");

	ICPS_GET_MONEY_FROM_TOTALFILE_JB(fp, sdb_icps_jb_total.paid_int_amt, "\npaid_int_amt", "实还正常利息（表内）");
	ICPS_GET_MONEY_FROM_TOTALFILE_JB(fp, sdb_icps_jb_total.non_to_accrued_ovd_prin_amt, "\nnon_to_accrued_ovd_prin_amt", "逾期本金（非应计）转逾期本金（应计）");
	ICPS_GET_MONEY_FROM_TOTALFILE_JB(fp, sdb_icps_jb_total.non_to_accrued_prin_amt, "\nnon_to_accrued_prin_amt", "正常本金（非应计）转正常本金（应计）");
	ICPS_GET_MONEY_FROM_TOTALFILE_JB(fp, sdb_icps_jb_total.paid_non_accrued_ovd_prin_amt, "\npaid_non_accrued_ovd_prin_amt", "实还逾期本金（非应计）");
	ICPS_GET_MONEY_FROM_TOTALFILE_JB(fp, sdb_icps_jb_total.paid_out_int_amt, "\npaid_out_int_amt", "实还正常利息（表外）");
	ICPS_GET_MONEY_FROM_TOTALFILE_JB(fp, sdb_icps_jb_total.accrued_to_non_prin_amt, "\naccrued_to_non_prin_amt", "正常本金（应计）转正常本金（非应计）");
	ICPS_GET_MONEY_FROM_TOTALFILE_JB(fp, sdb_icps_jb_total.accrued_to_non_ovd_prin_amt, "\naccrued_to_non_ovd_prin_amt", "逾期本金（应计）转逾期本金（非应计）");
	ICPS_GET_MONEY_FROM_TOTALFILE_JB(fp, sdb_icps_jb_total.paid_prin_amt, "\npaid_prin_amt", "实还正常本金（应计）");
	ICPS_GET_MONEY_FROM_TOTALFILE_JB(fp, sdb_icps_jb_total.paid_non_accrued_prin_amt, "\npaid_non_accrued_prin_amt", "实还正常本金（非应计）");
	ICPS_GET_MONEY_FROM_TOTALFILE_JB(fp, sdb_icps_jb_total.in_to_out_ovd_int_amt, "\ninternal_to_out_ovd_int_amt", "逾期利息（表内）转逾期利息（表外）");
	ICPS_GET_MONEY_FROM_TOTALFILE_JB(fp, sdb_icps_jb_total.in_to_out_int_amt, "\ninternal_to_out_int_amt", "正常利息（表内）转正常利息（表外）");
	ICPS_GET_MONEY_FROM_TOTALFILE_JB(fp, sdb_icps_jb_total.paid_in_ovd_int_amt, "\npaid_internal_ovd_int_amt", "实还逾期利息（表内）");
	ICPS_GET_MONEY_FROM_TOTALFILE_JB(fp, sdb_icps_jb_total.paid_out_ovd_int_amt, "\npaid_out_ovd_int_amt", "实还逾期利息（表外）");
	ICPS_GET_MONEY_FROM_TOTALFILE_JB(fp, sdb_icps_jb_total.encash_amt, "\nencash_amt", "当天贷款发放金额");
	ICPS_GET_MONEY_FROM_TOTALFILE_JB(fp, sdb_icps_jb_total.accrued_ovd_int_pnlt_amt, "\naccrued_ovd_int_pnlt_amt", "短期逾期90天以内（含）贷款计提每日逾期利息罚息");
	ICPS_GET_MONEY_FROM_TOTALFILE_JB(fp, sdb_icps_jb_total.non_accrued_ovd_int_pnlt_amt, "\nnon_accrued_ovd_int_pnlt_amt", "短期逾期90天以上贷款计提每日逾期利息罚息");
	ICPS_GET_MONEY_FROM_TOTALFILE_JB(fp, sdb_icps_jb_total.prin_to_ovd_prin_amt, "\nprin_to_ovd_prin_amt", "正常本金转逾期本金");
	ICPS_GET_MONEY_FROM_TOTALFILE_JB(fp, sdb_icps_jb_total.int_to_ovd_int_amt, "\nint_to_ovd_int_amt", "正常利息转逾期利息");
	ICPS_GET_MONEY_FROM_TOTALFILE_JB(fp, sdb_icps_jb_total.non_accrued_int_amt, "\nnon_accrued_int_amt", "短期逾期90天以上贷款计提每日利息");
	ICPS_GET_MONEY_FROM_TOTALFILE_JB(fp, sdb_icps_jb_total.paid_in_ovd_int_pnlt_int_amt, "\npaid_internal_ovd_int_pnlt_int_amt", "实还逾期利息罚息（表内）");
	ICPS_GET_MONEY_FROM_TOTALFILE_JB(fp, sdb_icps_jb_total.paid_out_ovd_int_pnlt_int_amt, "\npaid_out_ovd_int_pnlt_int_amt", "实还逾期利息罚息（表外）");
	ICPS_GET_MONEY_FROM_TOTALFILE_JB(fp, sdb_icps_jb_total.accrued_int_amt, "\naccrued_int_amt", "短期正常/逾期90天以内（含）贷款计提每日利息");
	ICPS_GET_MONEY_FROM_TOTALFILE_JB(fp, sdb_icps_jb_total.out_to_in_ovd_int_amt, "\nout_to_internal_ovd_int_amt", "逾期利息（表外）转逾期利息（表内）");
	ICPS_GET_MONEY_FROM_TOTALFILE_JB(fp, sdb_icps_jb_total.out_to_in_int_amt, "\nout_to_internal_int_amt", "正常利息（表外）转正常利息（表内）");
	ICPS_GET_MONEY_FROM_TOTALFILE_JB(fp, sdb_icps_jb_total.accrued_ovd_prin_pnlt_amt, "\naccrued_ovd_prin_pnlt_amt", "短期逾期90天以内（含）贷款计提每日逾期本金罚息");
	ICPS_GET_MONEY_FROM_TOTALFILE_JB(fp, sdb_icps_jb_total.non_accrued_ovd_prin_pnlt_amt, "\nnon_accrued_ovd_prin_pnlt_amt", "短期逾期90天以上贷款计提每日逾期本金罚息");
	ICPS_GET_MONEY_FROM_TOTALFILE_JB(fp, sdb_icps_jb_total.paid_out_ovd_prin_pnlt_int_amt, "\npaid_out_ovd_prin_pnlt_int_amt", "实还逾期本金罚息（表外）");
	ICPS_GET_MONEY_FROM_TOTALFILE_JB(fp, sdb_icps_jb_total.paid_in_ovd_prin_pnlt_int_amt, "\npaid_internal_ovd_prin_pnlt_int_amt", "实还逾期本金罚息（表内）");
	ICPS_GET_MONEY_FROM_TOTALFILE_JB(fp, sdb_icps_jb_total.out_to_in_ovd_prin_pnlt_amt, "\nout_to_internal_ovd_prin_pnlt_amt", "逾期本金罚息（表外）转逾本金罚息（表内）");
	ICPS_GET_MONEY_FROM_TOTALFILE_JB(fp, sdb_icps_jb_total.out_to_in_ovd_int_pnlt_amt, "\nout_to_internal_ovd_int_pnlt_amt", "逾期利息罚息（表外）转逾期利息罚息（表内）");
	ICPS_GET_MONEY_FROM_TOTALFILE_JB(fp, sdb_icps_jb_total.in_to_out_ovd_int_pnlt_amt, "\ninternal_to_out_ovd_int_pnlt_amt", "逾期利息罚息（表内）转逾期利息罚息（表外）");
	ICPS_GET_MONEY_FROM_TOTALFILE_JB(fp, sdb_icps_jb_total.in_to_out_ovd_prin_pnlt_amt, "\ninternal_to_out_ovd_prin_pnlt_amt", "逾期本金罚息（表内）转逾期本金罚息（表外）");
	ICPS_GET_MONEY_FROM_TOTALFILE_JB(fp, sdb_icps_jb_total.paid_accrued_ovd_prin_amt, "\npaid_accrued_ovd_prin_amt", "实还逾期本金（应计）");
	ICPS_GET_MONEY_FROM_TOTALFILE_JB(fp, sdb_icps_jb_total.withhold_amt, "\nwithhold_amt", "当天网商备付金代扣金额");
	ICPS_GET_MONEY_FROM_TOTALFILE_JB(fp, sdb_icps_jb_total.refund_amt, "\nrefund_amt", "当天退回网商备付金金额");
	ICPS_GET_MONEY_FROM_TOTALFILE_JB(fp, sdb_icps_jb_total.xpt_in_int_amt, "\nexempt_int_amt", "减免正常利息（表内）");
	ICPS_GET_MONEY_FROM_TOTALFILE_JB(fp, sdb_icps_jb_total.xpt_out_int_amt, "\nexempt_out_int_amt", "减免正常利息（表外）");
	ICPS_GET_MONEY_FROM_TOTALFILE_JB(fp, sdb_icps_jb_total.xpt_in_ovd_int_amt, "\nexempt_internal_ovd_int_amt", "减免逾期利息（表内）");
	ICPS_GET_MONEY_FROM_TOTALFILE_JB(fp, sdb_icps_jb_total.xpt_out_ovd_int_amt, "\nexempt_out_ovd_int_amt", "减免逾期利息（表外）");
	ICPS_GET_MONEY_FROM_TOTALFILE_JB(fp, sdb_icps_jb_total.xpt_in_ovd_prin_pnlt_int_amt, "\nexempt_internal_ovd_prin_pnlt_int_amt", "减免逾期本金罚息（表内）");
	ICPS_GET_MONEY_FROM_TOTALFILE_JB(fp, sdb_icps_jb_total.xpt_out_ovd_prin_pnlt_int_amt, "\nexempt_out_ovd_prin_pnlt_int_amt", "减免逾期本金罚息（表外）");
	ICPS_GET_MONEY_FROM_TOTALFILE_JB(fp, sdb_icps_jb_total.xpt_in_ovd_int_pnlt_int_amt, "\nexempt_internal_ovd_int_pnlt_int_amt", "减免逾期利息罚息（表内）");
	ICPS_GET_MONEY_FROM_TOTALFILE_JB(fp, sdb_icps_jb_total.xpt_out_ovd_int_pnlt_int_amt, "\nexempt_out_ovd_int_pnlt_int_amt", "减免逾期利息罚息（表外）");
	ICPS_GET_MONEY_FROM_TOTALFILE_JB(fp, sdb_icps_jb_total.fee_amt, "\nfee_amt", "平台服务费汇总金额");
    /*插入数据库*/
...
int ffunc_init_jiebei_file_info(HXMLTREE lXmlhandle, PARA_FILEINFOINIT st_para_fileinfoinit)
{
	int iRet;

	/*汇总记账文件*/
	iRet = ffunc_ins_file_info(st_para_fileinfoinit.busi_date, st_para_fileinfoinit.busi_no, "accounting");
    /*在途支用明细文件说明*/
    iRet = ffunc_ins_file_info(st_para_fileinfoinit.busi_date, st_para_fileinfoinit.busi_no, "lending_detail");
    /*放款（合约）明细文件*/
    iRet = ffunc_ins_file_info(st_para_fileinfoinit.busi_date, st_para_fileinfoinit.busi_no, "loan_detail");
    /*放款（分期）明细文件*/
    iRet = ffunc_ins_file_info(st_para_fileinfoinit.busi_date, st_para_fileinfoinit.busi_no, "repay_plan");
    /*还款（合约）明细文件*/
    iRet = ffunc_ins_file_info(st_para_fileinfoinit.busi_date, st_para_fileinfoinit.busi_no, "repay_loan_detail");
    /*还款（分期）明细文件*/
    iRet = ffunc_ins_file_info(st_para_fileinfoinit.busi_date, st_para_fileinfoinit.busi_no, "repay_instmnt_detail");
    /*减免（合约）明细文件说明*/
    iRet = ffunc_ins_file_info(st_para_fileinfoinit.busi_date, st_para_fileinfoinit.busi_no, "exempt_loan_detail");
    /*减免（分期）明细文件说明*/
    iRet = ffunc_ins_file_info(st_para_fileinfoinit.busi_date, st_para_fileinfoinit.busi_no, "exempt_instmnt_detail");
    /*日初（合约）信息文件说明*/
    iRet = ffunc_ins_file_info(st_para_fileinfoinit.busi_date, st_para_fileinfoinit.busi_no, "loan_init");
    /*日初（分期）信息文件说明*/
    iRet = ffunc_ins_file_info(st_para_fileinfoinit.busi_date, st_para_fileinfoinit.busi_no, "instmnt_init");
    /*利息计提明细文件*/
    iRet = ffunc_ins_file_info(st_para_fileinfoinit.busi_date, st_para_fileinfoinit.busi_no, "loan_calc");
    /*合约形态转移流水文件说明*/
    iRet = ffunc_ins_file_info(st_para_fileinfoinit.busi_date, st_para_fileinfoinit.busi_no, "arg_status_change");
    /*网商银行资金对账文件说明*/
    iRet = ffunc_ins_file_info(st_para_fileinfoinit.busi_date, st_para_fileinfoinit.busi_no, "fund_detail");
    return 0;
}
